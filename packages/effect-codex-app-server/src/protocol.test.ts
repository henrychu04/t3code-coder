...
/** Fails the test when the wrapped effect does not settle within the timeout. */
const failOnHang = <A, E>(effect: Effect.Effect<A, E>): Effect.Effect<A, unknown> =>
  effect.pipe(
    Effect.timeoutOption("5 seconds"),
    Effect.flatMap(
      Option.match({
        onNone: () => Effect.die(new Error("expected the effect to settle, but it hung")),
...
it.layer(NodeServices.layer)("effect-codex-app-server protocol", (it) => {
  ...
  it.effect.each([1, 7, 1024])(
    ...
    (chunkSize) =>
      Effect.gen(function* () {
        ...
        const firstLine = encodeUnknownJsonString({
          method: "x/first",
          params: { text: "hé🙂" },
        });
        const secondLine = encodeUnknownJsonString({ method: "x/second", params: { value: 2 } });
        const finalLine = encodeUnknownJsonString({
          method: "x/final",
          params: { text: "最後" },
        });
        const bytes = encoder.encode(`${firstLine}\r\n${secondLine}\n${finalLine}`);
  ...
  it.effect("keeps correlating responses and notifications while a request handler is parked", () =>
    Effect.gen(function* () {
      const { stdio, input, output } = yield* makeInMemoryStdio();
      ...
      // Park the server-request handler (approval-style human decision).
      yield* Queue.offer(
        input,
        encodeJsonl({ id: 10, method: "item/commandExecution/requestApproval", params: {} }),
      );

      // While the handler is parked, notifications must still be delivered.
      yield* Queue.offer(
        input,
        encodeJsonl({ method: "item/agentMessage/delta", params: { delta: "still streaming" } }),
      );
      assert.deepEqual(yield* Deferred.await(notificationDeferred), [
        { method: "item/agentMessage/delta", params: { delta: "still streaming" } },
      ]);

      // ...and a client request must still round-trip.
      const pendingRead = yield* transport.request("account/read", {}).pipe(Effect.forkScoped);
      assert.deepEqual(yield* decodeJson(yield* Queue.take(output)), {
        id: 1,
        method: "account/read",
        params: {},
      });
      yield* Queue.offer(input, encodeJsonl({ id: 1, result: { account: null } }));
      assert.deepEqual(yield* failOnHang(Fiber.join(pendingRead)), { account: null });

      // Releasing the handler writes its response to the peer.
      yield* Deferred.succeed(releaseHandler, undefined);
      assert.deepEqual(yield* failOnHang(decodeJson(yield* Queue.take(output))), {
        id: 10,
  ...
  it.effect("fails requests issued after termination instead of hanging", () =>
    Effect.gen(function* () {
      const { stdio, input } = yield* makeInMemoryStdio();
      const termination = yield* Deferred.make<CodexError.CodexAppServerError>();
      const transport = yield* CodexProtocol.makeCodexAppServerPatchedProtocol({
      ...
      const error = yield* failOnHang(Effect.flip(transport.request("thread/start", {})));
      assert.instanceOf(error, CodexError.CodexAppServerInputStreamEndedError);

      // The raw inbound queues are ended too, so raw-stream consumers complete.
      yield* failOnHang(Stream.runDrain(transport.incomingNotifications));
      yield* failOnHang(Stream.runDrain(transport.incomingRequests));
    }),
  );

  it.effect("settles requests racing input termination", () =>
    Effect.gen(function* () {
      const { stdio, input } = yield* makeInMemoryStdio();
      const transport = yield* CodexProtocol.makeCodexAppServerPatchedProtocol({ stdio });
      ...
      const requests = yield* Effect.forEach(
        ...
        (_, index) =>
          Effect.flip(transport.request("thread/read", { threadId: `thread-${index}` })),
        { concurrency: "unbounded" },
      ).pipe(Effect.forkScoped);
      yield* Queue.end(input);

      const errors = yield* failOnHang(Fiber.join(requests));
      assert.lengthOf(errors, 100);
