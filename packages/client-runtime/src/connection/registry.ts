  ...
  const followStream: EnvironmentRegistry["Service"]["followStream"] = (environmentId, stream) =>
    Stream.concat(
      ...
      Stream.switchMap(
        Option.match({
          onNone: () => Stream.empty,
          onSome: () => runStream(environmentId, stream).pipe(Stream.catch(() => Stream.empty)),
        }),
      ),
    );
  ...
  const reconcilePlatform = Effect.fn("EnvironmentRegistry.reconcilePlatform")(function* (
    ...
    yield* mutationLock.withPermits(1)(
      Effect.gen(function* () {
        ...
        const active = yield* SubscriptionRef.get(serviceScopes);
        for (const [environmentId, current] of active) {
          const next = nextEntries.get(environmentId);
          if (next === undefined || !Equal.equals(current.entry, next)) {
            yield* closeServiceScope(environmentId);
          }
        }
        yield* SubscriptionRef.set(entries, nextEntries);
      }),
    );
    yield* Effect.forEach(
      registrations,
      (registration) =>
        acquireSupervisor(registration.target.environmentId).pipe(
  ...
  yield* Effect.addFinalizer(() =>
    SubscriptionRef.get(serviceScopes).pipe(
      Effect.flatMap((current) =>
        Effect.forEach(current.values(), (active) => Scope.close(active.scope, Exit.void), {
          discard: true,
        }),
      ),
    ),
