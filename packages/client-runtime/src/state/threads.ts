...
export const makeEnvironmentThreadState = Effect.fn("EnvironmentThreadState.make")(function* (
  ...
  if (resumeCache) {
    ...
    // Registered before the stream fibers so sizing happens once, after they
    // stop. Retaining active messages must not serialize every streaming delta.
    yield* Effect.addFinalizer(() =>
      Effect.sync(() => {
        if (resumeCache.owner === owner) resumeCache.onIdle?.();
      }),
    );
  }
  const cached =
...
export function createEnvironmentThreadStateAtoms<R, E>(
  ...
  const resumeFamily = Atom.family((key: string) =>
    Atom.make((get): ThreadResumeCache => {
      const retention = get(retentionAtom);
      const resume: ThreadResumeCache = {
        snapshot: undefined,
        owner: undefined,
        onActive: () => retention.release(resume),
        onIdle: () => retention.retain(resume),
      };
      get.addFinalizer(() => {
        retention.release(resume);
