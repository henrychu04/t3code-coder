...
// Presentation-only view of the existing text question lifecycle. No new
// attachment protocol or durable storage is needed for Coder questions.
const QuestionAnswer = Schema.Struct({
  requestId: Schema.String,
  questionTextById: Schema.Record(Schema.String, Schema.String),
...
export const isQuestionAnswer = Schema.is(QuestionAnswer);
...
export function foldUserInputActivities(
  ...
  for (const [requestId, group] of requests) {
    ...
    const answer = { requestId, questionTextById, answers };
    if (!isQuestionAnswer(answer)) continue;
    const submittedAnswer = Object.values(answers).some(getQuestionAnswerText);
...
export function getQuestionAnswerPreview(answer: QuestionAnswer): string {
  ...
  return (
    answers.length > 0 ? answers.join(" · ") : Object.values(answer.questionTextById).join(" · ")
  )
    .replace(/\s+/g, " ")
    .trim();
}
