...
`,
  ...
  it("rejects a latency sample measured for a replaced connection", async () => {
    const directory = await NodeFS.mkdtemp(NodePath.join(NodeOS.tmpdir(), "t3-coder-gateway-"));
    tempDirectories.push(directory);
    const configPath = NodePath.join(directory, "config.json");
    let closeConnection: (() => void) | undefined;
    ...
    const latencyPromise = request({
      url: `${gateway.url}/api/workspaces/project-one/latency`,
    });
    for (let attempt = 0; attempt < 100 && pendingPings.length === 0; attempt += 1) {
      await new Promise((resolve) => setTimeout(resolve, 5));
    }
    strictEqual(pendingPings.length >= 1, true);
    strictEqual(
      (
        await request({
          url: `${gateway.url}/api/workspaces/project-one/connection`,
    ...
    pendingPings.splice(0).forEach((respond) => respond());
    strictEqual((await latencyPromise).statusCode, 502);
    strictEqual(
      (
        await request({
          url: `${gateway.url}/api/workspaces/project-one/latency`,
