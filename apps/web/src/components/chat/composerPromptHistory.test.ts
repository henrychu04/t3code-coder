...
describe("recallableComposerPrompt", () => {
  it("strips send-time context blocks and the ultrathink prefix", () => {
    ...
    expect(recallableComposerPrompt(`Ultrathink:\n${withTerminal}`)).toBe("Investigate this");
  });

  it("does not recall pasted-image attachment IDs, but keeps project file links", () => {
    const image =
      "[image.png](/home/coder/.t3-coder/attachments/12345678-1234-4234-8234-123456789abc.png)";
    expect(recallableComposerPrompt(image)).toBe("");
    expect(recallableComposerPrompt(`Explain [index.ts](src/index.ts) ${image}`)).toBe(
      "Explain [index.ts](src/index.ts)",
    );
  });
