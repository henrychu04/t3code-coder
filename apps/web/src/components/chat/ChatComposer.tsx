...
import type { SessionPhase, Thread, ThreadShell } from "../../types";
...
const ComposerFooterModeControls = memo(function ComposerFooterModeControls(props: {
  ...
  const [runtimeModeOpen, setRuntimeModeOpen] = useComposerMenuState(props.hidden);
  ...
  return (
    <>
      ...
      <Tooltip>
        <Select
          open={runtimeModeOpen}
          onOpenChange={setRuntimeModeOpen}
...
export const ChatComposer = memo(function ChatComposer(props: ChatComposerProps) {
  ...
  const openPullRequest = useOpenPrLink(routeThreadRef);
      ...
      if (!terminalContextIdListsEqual(composerTerminalContexts, terminalContextIds)) {
        setComposerDraftTerminalContexts(
          ...
          syncTerminalContextsByIds(
            restoredTerminalContexts ?? composerTerminalContexts,
            terminalContextIds,
          ),
        );
      }
    ...
    (item: ComposerCommandItem) => {
      ...
      if (item.type === "pull-request") {
        if (
          ...
          return;
        const replacement = `${formatReviewCommentContext(buildPullRequestReferenceContext(item.pullRequest))} `;
  ...
  const promptHistoryTargetKey = clipboardUploadTarget;
  useEffect(() => {
    promptHistoryPositionRef.current = null;
