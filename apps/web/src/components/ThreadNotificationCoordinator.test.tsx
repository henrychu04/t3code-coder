...
describe("thread notifications", () => {
  ...
  it.each([
    ...
    "uses the same %s event for in-app alerts and background badges",
    async (event, title) => {
      state.mode = "sound";
      await render();
      state[event] = true;
  ...
  it("can play sounds without showing toasts or badges", async () => {
    state.focused = false;
    state.inApp = false;
    state.mode = "sound";
    await render();
  ...
  it("clears background badges on focus without asking the OS to show content", async () => {
    state.mode = "sound";
    state.focused = false;
    await render();
    await complete();
