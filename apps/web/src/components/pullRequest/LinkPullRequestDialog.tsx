...
/**
 * Which merge request an input names, or why it cannot. A URL carries its own host and
...
export function resolveLinkPullRequestInput(input: {
  ...
  if (input.project === null) {
    return { error: "Paste a full URL to link a merge request from another repository." };
  }
  ...
  if (webUrl === null || webReference === null) {
    return { error: "Paste a full URL; this project's host has no known merge request URL." };
  }
  return {
...
function LinkPullRequestDialog({
  ...
  const submit = useCallback(async () => {
    ...
    try {
      ...
      setSubmitError(error instanceof Error ? error.message : "Could not link the merge request.");
      return;
    } finally {
      setPending(false);
    }
  ...
  const validation = !dirty
    ...
    : reference.trim().length === 0
      ? "Paste a merge request URL or enter 123 / #123."
      : resolved === null
        ? "Use a merge request URL, 123, or #123."
        : "error" in resolved
          ? resolved.error
          : null;
  ...
  return (
    <Dialog open={open} onOpenChange={(next) => (pending ? undefined : onOpenChange(next))}>
      <DialogPopup className="max-w-xl">
        <DialogHeader>
          <DialogTitle>Link merge request</DialogTitle>
          <DialogDescription>
            Attach a merge request to this thread. A full URL can point at any repository on a host
