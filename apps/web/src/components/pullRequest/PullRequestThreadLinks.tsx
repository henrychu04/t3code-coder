...
function EnabledPullRequestThreadLinks({
  ...
  const changeLink = async (threadId: ThreadId, remove: boolean) => {
    ...
    try {
      ...
      toastManager.add({
        type: "error",
        title: remove ? "Could not unlink the merge request" : "Could not link the merge request",
      ...
      {display === "picker" ? (
        <Dialog open onOpenChange={onPickerOpenChange}>
          <DialogPopup className="max-w-md" showCloseButton={false}>
            <DialogTitle className="sr-only">Link merge request to a thread</DialogTitle>
            <ThreadPicker
              environmentId={environmentId}
              url={url}
              pending={pending}
