...
export function getProviderSummary(provider: ServerProvider | undefined) {
  if (!provider) {
    return {
      ...
      detail: "Waiting for the workspace to report this provider's availability.",
    };
  }
  if (!provider.enabled) {
    return {
      headline: "Disabled",
      detail:
        provider.message ?? "This provider is installed but disabled for new sessions in T3 Coder.",
    };
  }
  if (!provider.installed) {
    return {
      headline: "Not found",
      detail: provider.message ?? "This provider is not available in the workspace.",
    };
  }
  if (provider.auth.status === "unauthenticated") {
    return {
      headline: "Unavailable",
      detail: "Workspace access for this provider is not ready.",
    };
  }
