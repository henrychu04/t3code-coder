...
function EnvironmentScopeMenu({
  ...
  return (
    <ScopeMenu
      ...
      icon={selected ? <ServerIcon aria-hidden className="size-3.5 shrink-0" /> : null}
      label={
        selected
...
function CheckoutScopeMenu({ value, groups, onChange }: SettingsScopeBreadcrumbProps) {
  const group = groups.find((group) => group.projectKey === value.project);
  const members =
  ...
  return (
    <>
      ...
      <WorkspaceBreadcrumbItem className="min-w-0 shrink">
        <ScopeMenu
          ariaLabel="Settings checkout"
          icon={null}
          narrowed={!!value.checkout}
          label={
          ...
          <MenuRadioGroup
            ...
            onValueChange={(checkout) => {
              if (typeof checkout === "string")
                onChange({ ...value, checkout: checkout || undefined });
            }}
          >
            <MenuRadioItem value="">All checkouts</MenuRadioItem>
            <MenuSeparator />
            {members.map((member) => (
