...
vi.mock("../state/entities", () => ({
  readProjects: () => [
    {
      id: "project-remote",
      environmentId: "environment-coder",
...
describe("useNewThreadHandler", () => {
  it.each([
    ...
    const pendingOpen = openThread(
      { environmentId: "environment-coder", projectId: "project-remote" } as never,
      { replace: true },
    );

    testState.router.state.location.href = "/settings/preferences";
    testState.completeProjectFileRead(null);
    await pendingOpen;

    expect(testState.router.state.location.href).toBe("/settings/preferences");
