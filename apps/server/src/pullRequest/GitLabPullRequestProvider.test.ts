...
describe("getChangeRequest base freshness", () => {
  ...
  it.effect("replaces host write capabilities when the workspace probe fails", () =>
    Effect.gen(function* () {
      const changeRequest = yield* readWith({}, false);

      expect(changeRequest.capabilities).toMatchObject({
        diff: true,
        comment: false,
        actions: [],
        reactions: false,
      ...
      expect(changeRequest.viewerPermissions).toEqual({
        actions: [],
        comment: false,
        resolve: false,
        verdicts: [],
...
describe("expanding unchanged diff lines", () => {
  it.effect("reads exact revision file contents through the GitLab CLI", () =>
    Effect.gen(function* () {
      ...
      expect(getMergeRequestDiffFileContents).toHaveBeenCalledWith({
        cwd: "/w",
        repository: "acme/web",
        host: "gitlab.com",
        number: 7,
