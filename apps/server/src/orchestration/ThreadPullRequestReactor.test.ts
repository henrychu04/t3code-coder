...
const REPOSITORY_KEY = `gitlab.com/${REPOSITORY}`;
...
function summary(input: PullRequestRef, state: PullRequestDetail["state"]): PullRequestDetail {
  return {
    ...input,
    provider: "gitlab",
    title: "Merge request",
    ...
    capabilities: {
      ...
      mergeMethods: [],
      search: false,
      review: { verdicts: [], inlineComment: false, reply: false, resolve: false },
...
const project = {
  ...
  repositoryIdentity: {
    ...
    locator: {
      source: "git-remote",
      remoteName: "origin",
      remoteUrl: `git@gitlab.com:${REPOSITORY}.git`,
    },
  },
...
const makeHarness = Effect.fn("makeThreadPullRequestHarness")(function* (options: {
  ...
  readonly summary?: PullRequestService["Service"]["detail"];
    ...
    Layer.mock(PullRequestService)({
      detail: (input) =>
        Ref.update(summaryCalls, (calls) => [...calls, input]).pipe(
          Effect.andThen(options.summary?.(input) ?? Effect.succeed(summary(input, "open"))),
        ),
    }),
          ...
          branchPullRequest: ({ cwd, branch }) =>
            branch === "failed"
              ...
              : Effect.succeed({
                  ...branchPullRequest(),
                  repositoryKey:
                    branch === "wrong-repository" ? "gitlab.com/other/repository" : REPOSITORY_KEY,
                }),
        });
        yield* Effect.gen(function* () {
          yield* fixture.start();
            ...
            summary: (input) =>
              (changedRemote === "primary"
                ? Ref.set(identity, {
                    ...
                    canonicalKey: "gitlab.com/other/repository",
                    displayName: "other/repository",
                  })
                : Ref.set(detected, {
                    ...branchPullRequest(99),
                    repositoryKey: "gitlab.com/other/repository",
                    url: "https://gitlab.com/other/repository/-/merge_requests/99",
                  })
