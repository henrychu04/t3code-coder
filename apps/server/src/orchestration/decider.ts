...
function hasQueuedTurnStartForThread(
  ...
  for (const message of thread.messages) {
    ...
    if (messageAtMs === latestUserMessageAtMs) latestUserMessageAt = message.createdAt;
  }
  ...
  switch (command.type) {
    ...
    case "thread.user-input.respond": {
      ...
      if (
        ...
        // Commit the resolution and its follow-up message atomically. The
        // normal turn path steers a running agent or resumes an idle session.
        return yield* decideCommandSequence({
          readModel,
          commands: [
            {
