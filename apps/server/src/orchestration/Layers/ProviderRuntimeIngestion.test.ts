...
describe("ProviderRuntimeIngestion", () => {
  ...
  it.each([1, 25])(
    "projects %i screenshot references without hydrating artifact bytes",
    async (count) => {
      const harness = await createHarness();
      const artifacts = Array.from({ length: count }, (_, index) => ({
        id: ScreenshotArtifactId.make(`c56a4180-65aa-42ec-a945-${String(index).padStart(12, "0")}`),
  ...
  it("flushes buffered text before a blocking user question without item.completed", async () => {
    ...
    const thread = await waitForThread(
      ...
      (entry) =>
        ...
        entry.activities.some(
          (activity) => activity.id === "evt-user-input-requested-buffered-user-input-flush",
        ),
    );
    const message = thread.messages.find(
  ...
  it.each([true, false])(
    ...
    async (enableLegacyTokenStreaming) => {
      const harness = await createHarness({
        serverSettings: { responseStreamingMode: enableLegacyTokenStreaming ? "token" : "turn" },
      });
      const base = {
        provider: ProviderDriverKind.make("codex"),
        createdAt: "2026-01-01T00:00:00.000Z",
