...
const make = Effect.gen(function* () {
  ...
  const handleRevertRequested = Effect.fn("handleRevertRequested")(function* (
    ...
    const rollbackHistory = providerService.rollbackConversation({
      threadId: event.payload.threadId,
      numTurns: Math.max(0, currentTurnCount - event.payload.turnCount),
    });
    const shouldRollback = currentTurnCount > event.payload.turnCount;
    if (event.payload.restoreFiles !== false) {
      ...
      if (!checkpointExists && event.payload.turnCount !== 0) {
        yield* appendRevertFailureActivity({
          threadId: event.payload.threadId,
          turnCount: event.payload.turnCount,
          detail: "Filesystem checkpoint is unavailable.",
          createdAt: now,
        }).pipe(Effect.catch(() => Effect.void));
        return;
      }
      if (shouldRollback) yield* rollbackHistory;
  ...
  const processDomainEvent = Effect.fn("processDomainEvent")(function* (event: OrchestrationEvent) {
    if (event.type === "thread.turn-start-requested" || event.type === "thread.message-sent") {
      if (event.type === "thread.turn-start-requested") {
        pendingTurns.add(event.payload.threadId);
      }
  ...
  const processRuntimeEvent = Effect.fn("processRuntimeEvent")(function* (
    ...
    if (event.type === "turn.started") {
      ...
      const mayReplace = pendingTurns.has(event.threadId) && sameId(activeTurnId, turnId);
