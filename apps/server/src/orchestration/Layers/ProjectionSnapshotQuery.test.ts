    ...
    Effect.gen(function* () {
      ...
      yield* sql`
        ...
        SELECT 'turn-start-history:' || n, ${threadId}, 'old-turn:' || n, 'assistant',
          'Unrelated assistant output', 0, ${createdAt}, ${createdAt}
        FROM history
      `;
      yield* sql`
        INSERT INTO projection_thread_messages (
  ...
  it.effect("keeps compaction and queued-message eligibility in the turn-start query", () =>
    Effect.gen(function* () {
      ...
      yield* sql`
        INSERT INTO projection_thread_messages (
          message_id, thread_id, role, text, is_streaming, created_at, updated_at
        ) VALUES ('turn-start-other-user', ${threadId}, 'user', '/compact', 0,
          '2026-09-05T00:00:01.000Z', '2026-09-05T00:00:01.000Z')
      `;

      for (const { text, hasOtherUserMessages } of [
        ...
        yield* sql`
          UPDATE projection_thread_messages SET text = ${text}
