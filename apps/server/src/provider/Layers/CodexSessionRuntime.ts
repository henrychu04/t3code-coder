...
import { getCodexRuntimeModeConfig } from "../CodexRuntimeModes.ts";
    ...
    return {
      ...
      interruptTurn: (turnId) =>
        Effect.gen(function* () {
          ...
          // Bounded like the child interrupts above: a peer that accepts the
          // request but never answers must not hang Stop forever. A timeout
          // fails with a request error so callers see the interrupt did not
          // land instead of awaiting a response nothing will ever deliver.
          yield* client
            .request("turn/interrupt", {
              threadId: providerThreadId,
                  ...
                  onNone: () =>
                    Effect.fail(
                      CodexErrors.CodexAppServerRequestError.internalError(
                        "Codex App Server request 'turn/interrupt' timed out.",
                        undefined,
