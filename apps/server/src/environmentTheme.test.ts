...
it.layer(NodeServices.layer)("workspace theme bounds", (it) => {
  it.effect("caps the files examined and skips oversized files", () =>
    withEnvironmentThemes(
      Object.fromEntries([
        ["a-too-large.json", " ".repeat(32 * 1024 + 1)],
        ...Array.from({ length: 40 }, (_, index) => [
          `theme-${String(index).padStart(2, "0")}.json`,
          encodeThemeFile(NIGHTFALL_THEME),
        ]),
      ]),
      Effect.gen(function* () {
        assert.equal((yield* currentThemes).length, 31);
      }),
    ),
  );
  it.effect.skipIf(!symlinksSupported)("rejects a symlinked themes directory", () =>
    withEnvironmentThemes(
      {},
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        const { environmentThemesDir } = yield* ServerConfig.ServerConfig;
        const outside = yield* fs.makeTempDirectoryScoped({ prefix: "t3-theme-outside-" });
        yield* fs.writeFileString(
          path.join(outside, "outside.json"),
          encodeThemeFile(NIGHTFALL_THEME),
        );
        yield* fs.remove(environmentThemesDir, { recursive: true });
        yield* fs.symlink(outside, environmentThemesDir);
        assert.deepEqual(yield* currentThemes, []);
      }),
    ),
  );
