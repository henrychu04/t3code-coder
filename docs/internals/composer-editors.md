...
Only replace editor content when the controlled text or terminal-context identities change. Replacing it for a cursor
...
the source and marker decorations are not content. The Coder composer accepts plain clipboard text and validated composer images only;
it does not transfer structured context records. Terminal, review, long-text, and image
chips retain the existing Coder source grammar. Image IDs refer to validated workspace
copies transferred through the Coder CLI; draft and queued image bytes stay in memory.
