...
GitLab asks in batches, so a batch it could not read leaves that batch's paths out.
The contract distinguishes an unavailable read from a confirmed deletion.
