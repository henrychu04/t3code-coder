...
it("allows types and schemas through the real Knip CLI without hiding runtime or file findings", () => {
  // Keeping the disposable project here gives it the same Effect installation as the shared package.
  const cwd = NodeFS.mkdtempSync(
    NodePath.join(import.meta.dirname, "../packages/shared/.knip-test-"),
  );
